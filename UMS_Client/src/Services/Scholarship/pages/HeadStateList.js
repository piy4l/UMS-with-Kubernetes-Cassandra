...
const HeadStateList = () => {
    ...
    useEffect(() => {
        const getData = async () =>{
          ...
          let temp = {"Applied":0, "Advisor":1, "Hall":2, "Head":3, "Accepted":4, "Rejected":5};
          let ret = -1;
          for(let i=0; i<scholarship_list.length; i++){
            ret = Math.max(ret, temp[ scholarship_list[i].location ]);
          }            
          if(ret == 2) setDone(false);
        }
